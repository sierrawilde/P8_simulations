...
def modify_config(path, signal_power, locust_egg, locust_root, fake_track_random_seed, mean_slope):
    ...
    config['fake-track']['slope-std'] = 0
    with open(path, 'w') as outfile:
        json.dump(config, outfile)

def RunKatydidSimulation(n_sims, working_dir, katydid_config_path, locust_config_path, max_snr, min_snr, remove_egg, snr_file_path=None, min_slope=0, max_slope=1):
    """
    ...
    for ii_sim in range(n_sims):
        ...
        signal_slope.append(np.random.uniform(min_slope, max_slope))
        print(signal_slope[-1])
...
if __name__ == "__main__":
    ...
    print('--------RUNNING {} FAKE EVENT SIMULATIONS with uniform slope!--------\n'.format(args.n_sims))
    RunKatydidSimulation(args.n_sims, simulation_path, new_katydid_config_path, new_locust_config_path, args.max_snr, args.min_snr, args.remove_egg, args.snr_file_path, args.from_slope, args.to_slope)
